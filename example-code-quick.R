# define UDF fit.tvARMCMCunbd
source('TVBARC.R')
# define UDF fit.tvINGARCHMCMCcombo
source('TVINGARCH.R')
...
fit      <- fit.tvARMCMCunbd(data, order= 1 , Total_itr = MC.iter,     burn = MC.burn)
...
fitS <- fit.tvINGARCHMCMCcombo(as.numeric(c(x0,x)), order1 = 1, order2 = 1, norder = 4, knot = 6, Total_itr = MC.iter,  burn =MC.burn)

print("done!")

print(length(fitS$Mfn[[1]]))
...
save.image(file = "TVBARC-quick.Rdata")
